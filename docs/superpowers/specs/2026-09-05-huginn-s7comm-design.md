...
- 상태: **A단계 완료 · B단계 설계 승인** (2026-09-05). rev7 · 리뷰 5라운드 · 블로커 23건 반영
...
### B단계 — CONTROL (설계 승인 2026-09-05, 상세는 §10)
...
A단계와 갭 이후 구간 처리를 마친 뒤에 붙인다. **핵심 결정은 §2·§5 에 이미 있고**, 여기서는 구현으로 옮길 때 필요한 것만 더한다.

### 바꾸는 것은 표 두 개다

**`S7Access`** 가 여덟 코드를 `UNDECIDABLE` 에서 `CONTROL` 로 옮긴다:

| 함수코드 | 뜻 | | 함수코드 | 뜻 |
|---|---|---|---|---|
| `0x28` | PLC Control | | `0x1C` | Download ended |
| `0x29` | PLC Stop | | `0x1D` | Start upload |
| `0x1A` | Request download | | `0x1E` | Upload |
| `0x1B` | Download block | | `0x1F` | End upload |

`Reconciler` 는 `CONTROL` 을 `WRITE` 와 같은 **HIGH** 로 친다(1차 §5). 미등록 호스트의 PLC 정지·블록 다운로드가 최고 심각도로 뜬다는 뜻이고, 그것이 이 단계의 목적이다.

**`S7ObjectRef`** 가 그 코드들에 정적 이름을 낸다 — `fc:41` 대신 `plc-stop`, `fc:26` 대신 `download-request` 식이다. **파싱이 없으므로 틀릴 여지가 없다.** 운영자가 리포트를 읽을 수 있게 하는 최소한이며, *무엇을* 바꾸는지(어떤 블록인지)는 여전히 모른다.

> **함정 하나.** `0x1C`·`0x1D` 는 S7ANY 의 area 코드(카운터 C·타이머 T)와 값이 같다. 자리가 달라(함수코드 대 area) 충돌하지 않지만 두 표가 같은 파일에 있으므로 주석으로 못박는다.

### 서브서비스는 여전히 해석하지 않는다

`0x28` 의 실제 동작은 가변길이 서비스 문자열(`P_PROGRAM`·`_INSE`·`_DELE`)에 있고, 블록 함수의 대상 블록은 파일명 형태의 식별자에 있다. 둘 다 **읽지 않는다** — 실캡처가 없어 tshark 로 대조할 수 없고, 그러면 테스트가 구현이 아니라 픽스처를 검증하게 된다. 1차가 FC 8(Diagnostics)을 서브함수 없이 통째로 CONTROL 로 올린 것과 같은 보수적 판단이며, **같은 이유로 과대분류인 면이 있다는 것을 인정한다.**

### 검증 — 합성뿐이고 그렇게 적는다

- `S7Fixtures` 에 `plcStop()`·`downloadRequest()` 를 더한다
- `cli` E2E 에 **"미등록 호스트가 PLC 를 정지시킨다"** 시나리오 하나 — HIGH 위반과 종료 코드 1 이 나오는지가 실질 검증이다
- `samples/README.md` 의 "아직 검증되지 않은 것" 에서 CONTROL 항목을 **지우지 않는다.** 합성으로 옮겨졌을 뿐 실캡처 검증은 여전히 없다

### 회귀 가드 — 실캡처 수치가 하나도 안 움직여야 한다

4SICS 세 캡처에 이 여덟 함수코드는 **0 건**이다(실측). 그러므로 228 테스트와 세 캡처의 모든 수치(위반·대화 계수·미관측 바이트)가 그대로여야 한다. **하나라도 움직이면 매핑 외의 무언가를 건드린 것이다.**

A단계의 `S7AccessTest.A단계에서_제어_계열은_아직_UNDECIDABLE이다` 는 CONTROL 을 단언하도록 뒤집힌다 — 그 테스트의 주석이 *"B단계에서 CONTROL 로 올린다"* 고 예고해 둔 계획된 변경이다.

### 이 단계가 틀렸다고 판명되는 조건

- **실캡처에서 이 함수코드가 나타났는데 서브서비스를 안 봐서 과대분류가 실제 오탐을 만들면** → 통째로 CONTROL 로 올린 판단이 틀린 것이다. 지금은 표본이 0 이라 **시험 자체가 불가능하다는 것**도 함께 기록한다
- **실캡처 수치가 움직이면** → 매핑 외의 것을 건드린 것이다
