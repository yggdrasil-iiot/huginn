...
*(2026-09-05 해소.)* 이음매 판정이 끝난 뒤 리포트에 **프로토콜별 커버리지 표**를 붙였다 — 행이 프로토콜(해독기 등록 순서), 열이 해독 대화·UNDEC 대화·관찰·UNDEC 관찰·미관측 바이트다. **대화를 하나도 주장하지 못한 프로토콜도 0 행으로 낸다**: 151020 의 `MODBUS_TCP 0` 이야말로 1차가 산문으로만 적은 "이 캡처엔 Modbus 가 한 프레임도 없다" 를 리포트가 직접 말하는 것이다. 판정은 바뀌지 않는다 — 위반도 종료 코드도 그대로이고 리포트만 는다.
