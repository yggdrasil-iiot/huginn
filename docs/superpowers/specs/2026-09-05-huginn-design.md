...
- **갭 이후를 버리는 재조립 정책이 실캡처에서 대가를 치른다 (2026-09-05, 2차에서 발견).**
  ...
  관찰 비율(0.09%/60.88%/64.18%)이 첫 갭까지의 커버리지(0.01%/60.84%/64.14%)와 소수점 둘째
  자리까지 일치해 **차이가 전량 이 원인으로 설명된다.** 더 나쁜 것은 **리포트가 이 사실을 말하지
  않는다**는 점이다 — "해독한 대화 2" 로 보이지만 그 대화의 99.99% 를 못 봤다. 다음 회차 1순위:
  갭 이후 구간을 별도 실행(run)으로 넘겨 프레이머가 각 구간 시작부터 다시 시도하게 하거나,
  최소한 버린 바이트 수를 리포트에 내는 것. 수치는 `samples/README.md`.
